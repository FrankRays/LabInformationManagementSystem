...
<span id=\"content\">";
	...
	if($usercode==2)
	{
		echo "<div align=\"center\" style=\"font-size:14px; color:#1E7ACE; font-weight:bold;\">{$table_title}您负责的实验室相关的实验室利用率一览表</div>";
	}
	else
	{
		echo "<div align=\"center\" style=\"font-size:14px; color:#1E7ACE; font-weight:bold;\">{$table_title}实验室利用率一览表</div>";
	}
	...
	if($usercode==2)//负责人
	{
		...
		$sql_like = substr($sql_like,0,strlen($sql_like)-4);//去掉最后四个字符，即OR和两个空格
			$sql = "SELECT a_rname , a_cname, a_grade, a_major, a_class, sum( a_stime ) AS 实验课时, a_people AS 人数, sum( a_stime ) * a_people AS 人时数  FROM apply1 WHERE  a_id IN (SELECT a_id FROM `time` WHERE ".$sql_like.") AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}' GROUP BY a_rname , a_cname  order by a_rname";
	...
	{
		$sql = "SELECT a_rname , a_cname, a_grade, a_major, a_class, sum( a_stime ) AS 实验课时, a_people AS 人数, sum( a_stime ) * a_people AS 人时数  FROM apply1 WHERE a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}' GROUP BY a_rname , a_cname  order by a_rname";
		$result = mysql_query ( $sql ) or die ( "不能查询指定的数据库表：" . mysql_error() );
		...
		FROM  apply1 WHERE a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}' GROUP BY a_rname , a_cname
		) AS s";
