...
/*作者：陈灿*/
	...
	/*-----------------------引入房间数组END------------------------------*/
    	...
    	for ($room_code=0 ; $room_code<$room_arr_total_num ; $room_code++)
    	{
    		...
    		$sql = sprintf("SELECT a_grade , a_major , a_class , a_cname , a_rname , a_sweek FROM `apply` WHERE a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%' AND a_room LIKE '%%%s%%' AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}'",$var_sclass ,$room_arr[$room_code] );    
...
-->
  ...
  <?php
  //表格主体输出  
  for($xinqi=1 ; $xinqi<6 ; $xinqi++)	//进入星期的循环
  {
