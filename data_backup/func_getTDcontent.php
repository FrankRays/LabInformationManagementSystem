<?php
	...
	for($i_4=0; $i_4<$teachers_course_arr_num ; $i_4++)
	{
		...
		for($i_5=0; $i_5<$temp_course_arr_num ; $i_5++)
		{
			...
			$sql_3 = sprintf("SELECT a_sweek , a_id FROM `time` WHERE a_id IN (SELECT a_id FROM `apply1` WHERE a_rname='{$var_rname}' AND a_cname='{$var_cname}' AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}') AND a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%'   AND a_room LIKE '%%%s%%'" ,$var_sclass ,$room_arr[$room_code]);
			...
			$teacher_course_week_arr[] = array($row_apply[0],$row_apply[1],$row_apply[2],$var_cname,$var_rname,$course_week_arr);//存放结果的数组
