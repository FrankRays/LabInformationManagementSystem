<?php
	...
	//$room_code = '8B403';房间号，被引入时注意取消

		
    /*---------------------根据“星期”＋“节次”＋“房间号”提取教师名字BEGIN--------------------------*/
	//此部分提供的变量是$teachers_arr老师名称数组以及$teachers_arr_num该数组内元素的个数
			
	$sql_1 = sprintf("SELECT DISTINCT a_rname FROM apply1 WHERE a_id IN ( SELECT a_id FROM `time` WHERE a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%' AND a_room LIKE '%%%s%%' ) AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}'" ,$var_sclass ,$room_code);
	...
	for($i_2=0 ; $i_2<$teachers_arr_num ; $i_2++)
	{
		...
		$sql_2 = sprintf("SELECT DISTINCT a_cname FROM apply1 WHERE a_rname='{$var_rname}' AND  a_id IN ( SELECT a_id FROM `time` WHERE  a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%' AND a_room LIKE '%%%s%%') AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}'" ,$var_sclass ,$room_code);
	...
	for($i_4=0; $i_4<$teachers_course_arr_num ; $i_4++)
	{
		...
		for($i_5=0; $i_5<$temp_course_arr_num ; $i_5++)
		{
			...
			//$sql_3 = sprintf("SELECT a_grade , a_major , a_class , a_cname , a_rname , a_sweek FROM `apply1` WHERE a_rname='{$var_rname}' AND a_cname='{$var_cname}' AND  a_id IN ( SELECT a_id FROM `time` WHERE  a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%' AND a_room LIKE '%%%s%%') AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}'" ,$var_sclass ,$room_code);
			$sql_3 = sprintf("SELECT a_sweek ,a_id FROM `time` WHERE a_id IN ( SELECT a_id FROM `apply1` WHERE a_rname='{$var_rname}' AND a_cname='{$var_cname}' AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}') AND  a_sdate=$var_sdate AND a_sclass LIKE '%%%s%%' AND a_room LIKE '%%%s%%'" ,$var_sclass ,$room_code);
			
			$result_3 = mysql_query ( $sql_3 ) or die ( "SQL语句执行失败2222:" . mysql_error() );
			...
			//$teacher_course_week_arr[] = array($temp_row_arr_3[0],$temp_row_arr_3[1],$temp_row_arr_3[2],$temp_row_arr_3[3],$temp_row_arr_3[4],$course_week_arr);
			//取得----年级、专业、班级、课程名称、老师名称
			$sql_apply = "SELECT a_grade,a_major,a_class,a_cname FROM apply1 WHERE a_id='{$temp_row_arr_3[1]}'";

			$rs_apply = mysql_query( $sql_apply ) or die("SQL语句执行失败1212：" . mysql_error());
			$row_apply = mysql_fetch_row($rs_apply);
			$teacher_course_week_arr[] = array($row_apply[0],$row_apply[1],$row_apply[2],$row_apply[3],$var_rname,$course_week_arr);//存放结果的数组(年级、专业、班级、课程名称、老师名称)
	...
	for($i_7=0 ; $i_7<$teacher_course_week_arr_num ; $i_7++)//整个大数组的循环
	{
		...
		{			
			...
			{
				$td_content .= "<font color='red'>".$teacher_course_week_arr[$i_7][5][$i_8]."</font>/";
			}
				else 
				{
