...
function ShowLayer(t,l) {
	...
	if(oldObject==null)oldObject=getObject;//付值
		if(Layer.style.visibility != "visible" && oldObject==getObject){//判断是否操作同一表格
				//if(l>480)l=l-131;
				Layer.style.top = t + document.documentElement.scrollTop;
				Layer.style.left = l + document.documentElement.scrollLeft;
				Layer.style.visibility = "visible";
				oldObject=getObject;
								}
...
function getLayer(){
	...
	document.write("<div id=\"Layer\"  onclick=show() style=\"position:absolute; width:200; z-index:2; border:1px inset black; background-color: #f0f8ff; layer-background-color: #f0f8ff; visibility: hidden; height: 56; left:58; top:137;\"><input type=\"hidden\" name=\"ret\"><table bgcolor=#99B6D0 width=200 style=\"cursor:move\" onmousedown=MDown(Layer)><tr align=center><td>请选择日期：</td></tr></table><table id=\"calendar\" cellpadding=\"0\" align=\"center\"><thead><tr><td colspan=7 align=CENTER><select id=\"month\" onChange=\"newCalendar()\" name=\"select\" class=\"smallSel\">");
	...
	document.write("</select><select id=\"year\" onChange=\"newCalendar()\" name=\"select\" class=\"smallSel\">");
	 for (var intLoop = today.year-6; intLoop < (today.year+6); intLoop++)
...
//显示css
document.write("<style>TABLE	{font-family:宋体,MS SONG,SimSun,tahoma,sans-serif; font-size:12pt;border:0px}.drpdwn	{font-family:宋体,MS SONG,SimSun,tahoma,sans-serif;font-size:12pt;color:#000066;background-color:#FFFFFF} Select.smallSel{    BACKGROUND-COLOR: #ffffff;    COLOR: #000080;    FONT-SIZE: 12pt} .normal{BACKGROUND: #ffffff} .today {font-weight:bold;BACKGROUND: #6699cc} .satday{color:green} .sunday{color:red} .days {font-weight:bold} .Arraw {color:#0000BB; cursor:hand; font-family:Webdings; font-size:12pt}</style>");
