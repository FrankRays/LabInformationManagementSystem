...
<?php
	...
	$a_cname = $_POST['a_cname'];//获取提交的课程名称
	$a_rname = $_POST['a_rname'];//获取本课程老师的名字
	
	//2017-3-7添加两个搜索条件
	$a_major = $_POST['a_major'];//专业
	$a_class = $_POST['a_class'];//班级
	
	if($a_cname == '') {
		$a_cname = $_GET['a_cname'];//获取提交的课程名称
		$a_rname = $_GET['a_rname'];//获取本课程老师的名字
		
		//2017-3-7添加两个搜索条件
		$a_major = $_GET['a_major'];//专业
		$a_class = $_GET['a_class'];//班级
	}
	
	
	$search_condition = 'a_rname'; //用于返回到上一个页面的“搜索条件”
	$search_content = $a_rname; //用于返回到上一个页面的“搜索内容”
	//echo "-------".$a_rname."-------".$a_cname;

	$sql = sprintf ( "SELECT * FROM apply1 WHERE a_rname='{$a_rname}' AND a_cname='{$a_cname}' AND a_major='{$a_major}' AND a_class='{$a_class}'  AND a_date BETWEEN '{$valid_time_range_begin_date}' AND '{$valid_time_range_end_date}'");
	
	$rs = mysql_query ( $sql ) or die ( "获取a_id时发生错误:" . mysql_error() );
	$row = mysql_fetch_array( $rs );

	/*查询出老师名字、课程名称、类别、教材、年级、专业、班别、人数、三个需求*/
	/*a_rname,a_cname,a_type*/
...
<div id="formwrapper"><!--表单外边框DIV_BEGIN-->
    ...
    <form action="course_register_deal.php" method="post" onSubmit="return Validator.Validate(this,3)">
        ...
        <p>
			...
			<?php //$row['a_sid']=$row['a_sid']+1; echo $row['a_sid']; ?>
            <input name="sid" type="text" value="" size="2" />
        </p>
