...
<?php
	...
	echo "<script language='javascript'>alert('你无权进行此操作！');
		...
		//教师姓名参数结合SQL语句进行查询操作
		$sql = "SELECT u_name AS 用户姓名, u_lname AS 登录id , u_type AS 用户权限,u_gender AS 用户性别, u_cellphone AS 联系方式, u_email AS 电子邮箱, u_id AS 操作 FROM user";//为下面的字段名
		...
		$row = mysql_fetch_array ( $result );    //跳过第一行，即管理员那一行
		...
		for ( $i = 0; $i < $row_num-1; $i++ )//减去管理员那一行
		{
		  ...
		  for ( $j = 0; $j < mysql_num_fields($result)-1; $j++ ) 
			...
			//<a href='javascript:if(confirm(\"您确实要删除该记录吗？\"))location=\"del_user_result.php?u_id=$row[6]\"'>删除</a>
