...
<?php
	...
	if ( isset ( $_POST["btnSubmit"] )) {    //在按下本页的提交按钮后执行以下操作(提交按钮的name是btnSubmit)
		...
		if(!empty($_POST["sysmanagerr_pri"]))
		{
			...
			$sysmanagerr_pri_arr_count = count($sysmanagerr_pri_arr);//生成实验室主任权限数组长度
		...
		*/
		...
		if( $teacher_pri_update == true || $labmanager_pri_update == true || $sysmanagerr_pri_update==true || $labdirector_pri_update==true)
		{
