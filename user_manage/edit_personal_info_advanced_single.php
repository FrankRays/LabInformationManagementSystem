...
<title>修改单个用户信息</title>
</head>
<body>

<div id="formwrapper"><!--表单外边框DIV_BEGIN-->
	<form action="edit_personal_info_advanced_single_result.php" method="post" onSubmit="return Validator.Validate(this,3)">
	<fieldset><!--表单内边框DIV_BEGIN-->
        <legend>用户信息修改</legend><!--表单内边框标题-->
		<p>
        ...
        <p>
		<label for="u_status">状&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;态： </label>
     
		<input type="radio" name="u_status" value="1" <?php if($row[u_status]==1) echo "checked" ;?> />已审核&nbsp;&nbsp;<input type="radio" name="u_status" value="0" <?php if($row[u_status]==0) echo "checked" ;?>/>待审核
        </p>
        ...
        <p id="uboxstyle">
		    ...
		    <select name="u_type">
		    <option value="1" <?php if($row['u_type']=='1'){echo 'selected';}?>>普通教师</option>
		    <option value="2" <?php if($row['u_type']=='2'){echo 'selected';}?>>实验室负责人</option>
		    <option value="3" <?php if($row['u_type']=='3'){echo 'selected';}?>>实验室主任</option>
			<option value="4" <?php if($row['u_type']=='4'){echo 'selected';}?>>管理员</option>
			<?php
			if($n>4)
			{
			?>
			<option value="5" <?php if($row['u_type']=='5'){echo 'selected';}?>>系统管理员</option>
			<?php
			}
			?>
		    </select>
	    ...
	    <p>
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;
	       <!-- <input type="button" onclick="javascript:history.back()" class="buttonSubmit" value="返回"/>  返回用户筛选页面-->
